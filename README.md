...
If you maintain a fork of pele (or you are the original maintainer), you'll iterate on `~/.claude/` locally with real project names (e.g. `<YourApp>iOS`, `just build-ios`, `~/Developer/pele/`) and periodically push improvements back into the public `core/` with those names decoupled to placeholders.

`scripts/sync-from-local.sh` automates the mechanical part: it `cp`s the right files, applies the established token-replacement dictionary (`<YourApp>iOS` → `<YourApp>iOS`, `<ChatModule>` → `<ChatModule>`, etc.), and flags everything that needs paragraph-level review. It's a first-pass tool, not a substitute for reading `git diff`.
