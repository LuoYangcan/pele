...
4. `~/.claude/rules/image-assets.md` —— iOS 图片资源走 TodayTheme + ThemeImageManager
...
2. **实现改动**（Edit / Write）
3. **该子任务结束后跑编译**：
   - iOS 改动：`just build-ios`（或 `xcodebuild -project apps/ios-app/TodayiOS.xcodeproj -scheme TodayiOS -configuration Debug -derivedDataPath build/DerivedData -destination "generic/platform=iOS Simulator" build`）
...
严格按 `image-assets.md` 放 TodayTheme，不要图省事丢业务模块。
