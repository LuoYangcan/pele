...
**与 executor mobile-mcp 验收的关系**：本 Step 4.5 是 generator 实现完的**自测**（保证交给 executor 前自己已经看过对照结果）；executor 的 mobile-mcp 冒烟是**独立的二次验收**，两层不冲突。generator 自测 pass 不代表 executor 一定 pass，但**自测 fail 还交付 = 浪费 executor 的一次重试名额**。
