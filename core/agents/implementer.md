...
2. 只写 ownership 内的文件。可能存在其他 writer：不得回滚或覆盖他人改动，发现并发变化先适配当前文件状态，冲突即停下报告。
3. material 决策（可观察行为、scope、架构、硬约束、验收标准，或改变语义/丢数据的 fallback、默认值、转换、跳过、丢弃）不得自行拍板：停下，把候选判断和依据作为 open question 返回 Root。
4. 可运行调用方允许的窄域检查协助迭代；不运行最终验证，不写共享审计日志，不 commit、不 merge、不 push、不开 PR。

## 返回

worktree 绝对路径、`base_ref`、`git status --short`、完整 changed/untracked paths、ownership 内 diff 摘要、已运行的窄域检查及结果、候选自作主张判断、未解决项。
