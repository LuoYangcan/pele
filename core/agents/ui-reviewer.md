...
5. 逐条用例分静态 / 动态跑：
   - 静态：1 次 `mobile_list_elements_on_screen`（核心采样） + 1 次 `mobile_save_screenshot`，对照 spec 描述算间距 / frame，容差 ±2pt
...
issues:                                # FAIL 时列；PASS 时为空
  - severity: blocking | warning
    issue_type: ui-frame-mismatch | ui-animation-mismatch | ui-crash | amendment-not-fulfilled | other
    spec_section: 4 | 9
...
**为什么走文件**：和 executor `.specs/<slug>-review.md` / generator → planner feedback 同理 —— 多 iter 累积、主 agent 不漏字段（特别是 `ui_dynamic_cases_skipped` / frames_dir / observations 这种主 agent 容易简化掉的边角）。
...
- **独立 subagent 而不是 executor 的子步骤**：UI 验收用 mobile-mcp，调用 cost 比 build/lint 高一个量级；独立 context 让 executor retry 循环不必每次重跑 UI。executor 编译失败时 ui-reviewer 不必跑、ui-reviewer fail 时 executor 不必重跑
- **与 executor 平行**：两个 agent 验不同维度，没强依赖（generator 编译失败时 executor 直接 FAIL、不会调起 ui-reviewer；generator 编译通过后两者可以并行跑）
- **显式触发**：UI 验收成本高（启 sim、装 app、录屏抽帧），默认不调；用户说"跑下 UI / UI 走查"才调起。避免 spec §4 漏写 UI 用例时白白跑一次 ui-reviewer
- **verdict blocking**：UI 间距 / 动画错是 must-fix，不该让用户在评论区"看着办"；走 generator 重试循环更贴合"做错就改"的预期
- **生成 `.specs/<slug>-ui-review.md` 而非 `<slug>-review.md`**：和 executor 的 review 文档分名分文件，避免覆盖；retry 时 generator 可以同时 Read 两个文件
- **静态 vs 动态分类、5.b 1+1 预算、5.c 录屏路径、降级而非硬失败**：全部依据 review-mobile-ui SKILL.md，本 agent 不重复决策、只按 SOP 跑
