...
4. **agent 代写**：用户确认需要更新、但 worktree 里没体现对应文档变更 → **不中止 `/openpr`、不要求用户重跑**，agent 直接接手写：
   ...
   **理由**：项目内的文档（CLAUDE.md / AGENTS.md / rules / skills / templates / docs）最终读者是 agent 自己，agent 比用户更清楚怎么写才能让下一次 session 触发对、按需加载对、不歧义。用户的角色是 review 方向 + 把关用词，不是写初稿。
...
> 注意：这两个目录应在 `.gitignore` 里，理论上不会进 commit。`rm -rf` 是为了保持 worktree 干净 + 避免下次切回来看到陈旧文件。**不要**清理其他目录或临时文件。

如果用户在 `/openpr` 前明确说"留着 review 报告 / UI 截图，我要看一下"，跳过这一步——但注意提醒用户后续自己删，否则下次 `/openpr` 会被自动清掉。

## push 与开 PR

> 项目可在 `~/.claude/settings.json` 加 PreToolUse hook 在 `git push` / `gh pr create` 前自动跑 lint check / test，失败时阻止命令执行。pele 默认不带这种 hook（项目工具命名各异），按需自加。
