...
- **触发**：用户提了写代码需求（新功能 / 修 bug / 改 UI / 重构 / 加 feature / 加测试，会落地 Edit/Write/NotebookEdit）；主 agent 第一动作加载本 skill，再调 Lead Planner（大需求可 fan-out 给 planner-worker，由 spec-integrator 单写正式 spec）→ 用户拍板 → generator → executor → PASS/FAIL（FAIL 自动重调 generator ≤3 次）。
