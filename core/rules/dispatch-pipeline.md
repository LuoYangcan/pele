...
> 设计取舍：executor 跑一轮成本高（spec / build / lint / AMD / UI / 硬约束 全跑一遍），review-fix 改动通常很小、generator 自己跑编译已经能挡住绝大多数破坏性回归。executor 既然第一次已经 PASS，就**不**为 review-fix 再跑一次 —— 节省 token。
...
- generator review-fix 期间**可能**改坏 spec §4-5 验收 / UI / 硬约束 —— 用户在 P4 时可以选择「再跑一次完整 executor」bypass 这条规则
- generator 必须遵守 `~/.claude/agents/generator.md` 自己的 SOP（编译验证、不扩范围、AMD append）—— 这是这层简化的前置假设
- 如果 review-fix 触及关键路径（核心算法 / 数据流 / 多模块联动），主 agent **应在报给用户时显式提示**「本轮 review-fix 改动较大，建议手动复核 / 或显式说『再跑一次 executor』」

**阶段 4 失败循环里默认仍然要跑 executor**（不受本规则影响）—— 那是 verdict==FAIL 后的修复，必须验证修对了；只有两条路径砍掉 retry executor：(1)「verdict==PASS 后的 review-fix」（本 Step P3）；(2)「阶段 4 lint-only 快速路径」—— 上轮 executor 返回 `lint_only_fail: true`（FAIL 唯一根因是 lint），本轮 generator 修完后跳过 retry executor、主 agent 自己检查 build/lint 状态（详见阶段 4 失败循环段），只用一次、失败退化到常规路径。
       ...
       and (not fast_path_used):
        ...
        调 generator，prompt 里追加：
          ...
          - **显式说**「上轮 executor 仅 lint 失败、build / spec / AMD / 硬约束 / mock 等全过；
            ...
            修完跑 `<your lint check recipe>`（如 `just check` / `swiftlint`）+ 项目 build 命令自验、把 build/lint 状态写进结构化结论；
...
> **lint-only 快速路径设计取舍**（与 Step P3 review-fix 同源）：executor 跑一轮成本高（spec / build / lint / AMD / UI / 硬约束 全跑一遍）。上轮 `lint_only_fail: true` 意味着除 lint 外所有维度都已过；本轮 generator 改的只是 `<your lint fix recipe>`（如 `just fix`）/ 行尾空格 / import 顺序等格式问题，重跑完整 executor 是浪费。让 generator 自己跑 `<your lint check recipe>`（如 `just check`）+ build 自验、主 agent 检查结论即可。**风险与兜底**：极端边角是 `<your lint fix recipe>` 自动改写改坏了语义（SwiftFormat 换行连锁触发其他规则、误删 `unused_variable` 时删掉实际有副作用的代码），跳过 executor 漏一次 —— 但 review-fix 子循环也接受了同等级 trade-off，且 lint-only 风险更低；fast path 只用一次、失败退化到常规路径兜底。

> **为什么 generator prompt 里指 review 文件路径而不是中转 issues 文本**：和阶段 2 generator → planner feedback 文件同理 —— 多 iter 累积、主 agent 不漏字段。详见 `~/.claude/agents/executor.md` Step 8 和 `~/.claude/agents/ui-reviewer.md` Step 5。
...
**为什么 review-fix 走 generator 自己 append 而不是 planner**：review issues 是用户在实现阶段追加的具体指令、且 generator 是唯一知道"怎么修"的角色；让 generator 自己 append AMD 既留下审计痕迹又避免 planner 多一道工序。planner 只在「用户决策同步」这种早期触发点参与 append AMD。
...
5. **§9 Amendments 与 §1-7 分离**：原始需求快照（§1-7）是 planner 一次性产出的对齐文档，反复改 §1-7 会破坏「回溯最初想要什么」的能力。实现阶段用户追加的 bug fix / 微调一律走追加式的 §9，让审计痕迹完整、原始 spec 不被污染 —— 同时给 executor 一个明确的「DONE 必验、TODO 跳过」口径，不会把"用户已撤销 / 还没做"和"做错了"混在一起
