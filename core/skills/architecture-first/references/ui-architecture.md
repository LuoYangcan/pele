...
> 项目 某 iOS monorepo 的 ChatVC 范式（UIKit + extension 切片 + `FooState.swift` + `FooViewModel.swift`）是 **MVVM 的轻量变体**——下面会专门对照说。
...
### 骨架（项目 某 iOS monorepo ChatVC 范式）
...
某 iOS monorepo 的 ChatVC 范式（`AGENTS.md` "ViewController 文件拆分约定"）就是 MVVM 的轻量变体：
...
└─ medium / large
    ...
    │   │   └─ MVVM + Coordinator (项目 <MyAppRouter>)
