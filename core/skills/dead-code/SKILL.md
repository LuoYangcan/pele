...
- **内置 `simplify` skill**：`simplify`（Codex 内置）改完代码后跑 review subagent 自动 fix；本 skill 是单维度（unused），且**不自动 fix**（generator override 例外见上）。可以串行用：`simplify` 跑完后再跑本 skill 做最后一轮 cleanup。
