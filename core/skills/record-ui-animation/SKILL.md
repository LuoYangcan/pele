...
Agent 拿到 FRAMES_DIR 后用 Read 工具看每一帧 (Codex 支持 PNG 多图输入)，
...
- ❌ 不出 GIF（Codex 不支持 GIF 输入、出了也只能给用户看，不在本 skill 责任范围；如果 caller 要给用户存档可以自己额外跑 `ffmpeg -i recording.mp4 -vf fps=10,scale=480:-1 -loop 0 output.gif`）
