...
description: 新需求先写 spec 再写代码的 SOP（主索引 + 子目录两层、10 节必含、iOS UI 改动专项、PreToolUse hook 硬卡）。触发——进了 .worktrees/<slug>/ 准备落地 Edit/Write 但 .specs/<slug>.md 还不存在；主要由 planner subagent 在 dispatch-pipeline 阶段 1 执行。Bypass——touch .specs/<slug>.skip。不触发——cwd 不在 .worktrees/ 下 / spec 已存在 / 改 meta 配置。
---
