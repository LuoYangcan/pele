...
8. **`<project-specific>` 按需打开 IDE / workspace（默认跳过）**：use-worktree 阶段**不**自动 open Xcode / IDE。理由：命令行 build（如 `<your iOS build recipe>` / `xcodebuild -workspace <YourApp>.xcworkspace -scheme <YourApp>iOS build`）已能验编译；Xcode 打开 workspace 会触发 `-workspace` 模式 SPM resolve、容易撞主仓库 macOS-only artifact 缺失（典型症状：`binary target 'Sparkle' could not be mapped to an artifact`），项目里如果有专门 skill 处理（参考 pele 私有 fork 的 `worktree-spm-bootstrap` 模式）就走它。用户/agent 后续明确说「在 Xcode 里调 / 看 UI / 开 IDE」时再 open，必要时按对应 skill 修一次 SPM resolve
