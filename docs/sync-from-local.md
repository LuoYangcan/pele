...
Pele is a **deliberate fork**: the maintainer's `~/.claude/` is personalized (real project names like `<YourApp>iOS`, `just build-ios`, `~/Developer/pele/`), while the published `pele/core/` is decoupled (placeholders like `<YourApp>iOS`, `<your iOS build recipe>`, `<pele-checkout>`) so any user can install pele on any project. This doc describes how to push improvements from the personal version into the public version without leaking project-internal names back in.
...
2. **Apply token replacement** for unambiguous real-name → placeholder mappings (<YourApp>iOS → \<YourApp\>iOS, <ChatModule> → \<ChatModule\>, etc.)
...
Fix: remove the line. If surrounding prose mentions `<DesignSystemPackage>` + `<ImageRegistry>`, rewrite to abstract `<DesignSystemPackage>` + `<ImageRegistry>` wording (PR #6 set the precedent).
...
```bash
SENSITIVE='某 iOS monorepo|<ChatModule>\b|<ProfileModule>|<DevicesModule>|<OnboardingModule>|<TasksModule>|<SkillsModule>\b|<TodayModule>|<ChannelsModule>|<ConnectorsModule>|<CoreModule>|<UIModule>\b|<DesignSystemPackage>|<NetworkingModule>|<AuthModule>|<MyAppRouter>|<TelemetryModule>|<CloudKitModule>|<FoundationModule>|<ModelModule>|<CacheModule>|<APISetModule>|<PackageA>|<YourApp>iOS|Today\.xcworkspace|<YourApp>iOS\.xcodeproj|Today\.app|<ImageRegistry>|<SystemToolsModule>'
...
2. **Dangling references**: Removing `image-assets.md` reference isn't just deleting the line — surrounding prose ("严格按 `image-assets.md` 放 <DesignSystemPackage>") needs paragraph-level rewriting to "项目自己的图片资源约定（如有；例 `<DesignSystemPackage>` + `<ImageRegistry>`）". The script can flag, not fix.
